...
graph TD
    subgraph "Azure DevOps"
        OData[OData Feed: User Stories, Features, Tasks, Bugs, Areas, Iterations]
    end
...
> [!NOTE]
> The database schema synchronizes work item details into flat tables. The **UserStories**, **Bugs**, and **Features** entities include the `StoryPoints` estimation property, extracted directly from the Azure DevOps Analytics OData feed.
...
1. **`--mode full`**  
   ...
   *Behavior*: Deletes **all** local database records of `UserStories`, `Features`, `Bugs`, and `Tasks` for the target projects and performs a fresh full synchronization starting from `WorkItemSyncDate`.  
...
## ODBC Driver to use SQL Lite

You will need this to connect your Power BI to SQL Lite.

1.Download from http://www.ch-werner.de/sqliteodbc/
2.Chose sqliteodbc_w64.exe and install.
3.Run ODBC Data Sources (64-bit).
4.Click on the "Add..." button.
5.Select "SQLite3 ODBC Driver" from the list.
6.Fill in the "Data Source Name "devops" and click "OK".


## Build and Publish

```powershell
dotnet publish AzureDevOpsToPoweBI.csproj -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true
```
