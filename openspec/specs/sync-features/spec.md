# sync-features Specification

## Purpose
The `sync-features` capability enables the synchronization of Azure DevOps Feature work items into local storage (SQLite or SQL Server). This supports advanced program-level tracking and reporting by mapping high-level Feature items alongside their child work items (such as User Stories, Bugs, and Tasks) to maintain proper hierarchy in BI and reporting dashboards.
